...
function transformToLis(obj){
  ...
  Object.keys(obj).map((value) => {
    newArr.push(`<li>${value}: ${obj[value]}</li>`);
  });
  return newArr;
}
...
const totalSum = (input) => {
  // Solution code here...
  return input.flat().reduce((value, index) => value + index,0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(arr => {
    return arr.filter(value => value % 5 === 0 && typeof(value) === 'number').map(power => Math.pow(2, power));
  });
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let filter = data.filter(value => value.gender === 'male' || value.gender === 'female');
  let newData = filter.map(value => {
    return value.name;});
...
let findShortest = (data) => {
  // Solution code here...
  let shortest = data.reduce((accumulator, value) => {
