...
// Solution code here...
  return arr.indexOf(arr.slice().sort((value, ind) => ind.length - value.length)[0]);
};
...
const findHappiness = (arr) => {
  // Solution code here...
  return arr.filter(newArr => newArr.includes(':)'));
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let re = /\d/g;
  return arr.map(newArr => newArr.match(re).join(''));
};
